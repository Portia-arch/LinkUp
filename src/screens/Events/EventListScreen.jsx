...
export default function EventListScreen() {
  ...
  // Track RSVPed events (store their unique title or id)
  ...
  const handleRSVP = (event) => {
    ...
    if (joinedEvents.includes(event.title)) {
      setJoinedEvents((prev) => prev.filter((t) => t !== event.title));
      Alert.alert('RSVP Cancelled', `You have unjoined "${event.title}".`);
    } else {
      setJoinedEvents((prev) => [...prev, event.title]);
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            const isJoined = joinedEvents.includes(item.title);
